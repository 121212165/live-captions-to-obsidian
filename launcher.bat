...
:: Open Live Captions — try direct exe first, then SendKeys, then keybd_event as last resort
powershell -NoProfile -Command ^
  "$p = \"$env:LOCALAPPDATA\Microsoft\WindowsApps\LiveCaptions.exe\";" ^
  "$ok = $false;" ^
  "if (Test-Path $p) { try { Start-Process $p; $ok = $true } catch {} };" ^
  "if (-not $ok) { try { Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.SendKeys]::SendWait('^{Win}{L}'); $ok = $true } catch {} };" ^
  "if (-not $ok) {" ^
